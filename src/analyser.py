...
def _build_prompt(deals: list[dict]) -> str:
    ...
    return (
        ...
        "1. OzBargain COMMUNITY VALIDATED deals — Australia's biggest deal community has "
        "already vetted these. Score baseline 7+. Only score lower if the original price "
        "looks clearly inflated (fake discount).\n"
        "2. Officeworks PRICE BEAT deals — Officeworks guarantees to beat any AU competitor "
        "by 5%, so if they're the cheapest it's likely the best price available in Australia. "
        "Score on absolute price competitiveness.\n"
        "3. Other retailers — must show a genuine price drop vs market. Be strict. "
        "Only score 7+ if the discount is real and the product is worth buying.\n\n"
        "WATCH OUT FOR:\n"
        "- Fake discounts: inflated 'original' prices to make % off look bigger\n"
        "- Full-price listings with no real discount\n"
        "- Refurbished/used items presented as new\n\n"
        f"{deals_text}\n"
        "Score guide:\n"
        ...
        "OzBargain community-validated → baseline 7 unless red flags.\n"
        "Officeworks cheapest in market → baseline 7.\n"
        "Other retailers → strict, 7+ only for real discounts (20%+ off market price)."
    )
